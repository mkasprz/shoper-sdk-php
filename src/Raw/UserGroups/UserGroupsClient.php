...
namespace Shoper\Sdk\Rest\UserGroups;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\UserGroups\Requests\ListUserGroupsRequest;
use Shoper\Sdk\Rest\UserGroups\Types\ListUserGroupsResponse;
...
use Shoper\Sdk\Rest\UserGroups\Requests\UserGroupInsert;
use Shoper\Sdk\Rest\Types\UserGroup;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\UserGroups\Requests\UserGroupUpdate;
    ...
    public function listUserGroups(ListUserGroupsRequest $request = new ListUserGroupsRequest(), ?array $options = null): ?ListUserGroupsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListUserGroupsResponse::fromJson($json);
            }
    ...
    public function createUserGroup(UserGroupInsert $request, ?array $options = null): int|UserGroup|null
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('integer', UserGroup::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
    ...
    public function getUserGroup(string $id, ?array $options = null): ?UserGroup
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/user-groups/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
