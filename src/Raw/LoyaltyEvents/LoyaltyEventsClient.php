...
namespace Shoper\Sdk\Rest\LoyaltyEvents;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\LoyaltyEvents\Requests\ListLoyaltyEventsRequest;
use Shoper\Sdk\Rest\LoyaltyEvents\Types\ListLoyaltyEventsResponse;
...
use Shoper\Sdk\Rest\LoyaltyEvents\Requests\LoyaltyEventInsert;
use Shoper\Sdk\Rest\Types\LoyaltyEvent;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;

class LoyaltyEventsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListLoyaltyEventsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListLoyaltyEventsResponse
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    ...
                    path: "webapi/rest/loyalty-events",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListLoyaltyEventsResponse::fromJson($json);
            }
    ...
    /**
     * @param LoyaltyEventInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function createLoyaltyEvent(LoyaltyEventInsert $request, ?array $options = null): int|LoyaltyEvent|null
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('integer', LoyaltyEvent::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
    ...
    public function getLoyaltyEvent(string $id, ?array $options = null): ?LoyaltyEvent
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/loyalty-events/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return LoyaltyEvent::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
