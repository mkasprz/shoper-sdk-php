...
namespace Shoper\Sdk\Rest\ProductStocks;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\ProductStocks\Requests\ListProductStocksRequest;
use Shoper\Sdk\Rest\ProductStocks\Types\ListProductStocksResponse;
...
use Shoper\Sdk\Rest\ProductStocks\Requests\ProductStockInsert;
use Shoper\Sdk\Rest\Types\ProductStock;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\ProductStocks\Requests\ProductStockUpdate;

class ProductStocksClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListProductStocksRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListProductStocksResponse
    ...
    public function listProductStocks(ListProductStocksRequest $request = new ListProductStocksRequest(), ?array $options = null): ?ListProductStocksResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListProductStocksResponse::fromJson($json);
            }
    ...
    /**
     * @param ProductStockInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getProductStock(string $id, ?array $options = null): ?ProductStock
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/product-stocks/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updateProductStock(string $id, ProductStockUpdate $request = new ProductStockUpdate(), ?array $options = null): int|ProductStock|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deleteProductStock(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
