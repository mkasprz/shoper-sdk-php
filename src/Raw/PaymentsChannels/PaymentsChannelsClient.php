...
namespace Shoper\Sdk\Rest\PaymentsChannels;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\PaymentsChannels\Requests\ListPaymentsChannelsRequest;
use Shoper\Sdk\Rest\PaymentsChannels\Types\ListPaymentsChannelsResponse;
...
use Shoper\Sdk\Rest\PaymentsChannels\Requests\PaymentChannelInsert;
use Shoper\Sdk\Rest\Types\PaymentChannel;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\PaymentsChannels\Requests\PaymentChannelUpdate;

class PaymentsChannelsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
     ...
     * } $options
     * @return ?ListPaymentsChannelsResponse
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    ...
                    path: "webapi/rest/payments/{$paymentId}/channels",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListPaymentsChannelsResponse::fromJson($json);
            }
    ...
    public function createPaymentChannel(string $paymentId, PaymentChannelInsert $request, ?array $options = null): int|PaymentChannel|null
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('integer', PaymentChannel::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
    ...
    public function getPaymentChannel(string $paymentId, string $id, ?array $options = null): ?PaymentChannel
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/payments/{$paymentId}/channels/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return PaymentChannel::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
