...
namespace Shoper\Sdk\Rest\SubscriberGroups;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\SubscriberGroups\Requests\ListSubscriberGroupsRequest;
use Shoper\Sdk\Rest\SubscriberGroups\Types\ListSubscriberGroupsResponse;
...
use Shoper\Sdk\Rest\SubscriberGroups\Requests\SubscriberGroupInsert;
use Shoper\Sdk\Rest\Types\SubscriberGroup;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\SubscriberGroups\Requests\SubscriberGroupUpdate;

class SubscriberGroupsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListSubscriberGroupsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListSubscriberGroupsResponse
    ...
    public function listSubscriberGroups(ListSubscriberGroupsRequest $request = new ListSubscriberGroupsRequest(), ?array $options = null): ?ListSubscriberGroupsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListSubscriberGroupsResponse::fromJson($json);
            }
    ...
    /**
     * @param SubscriberGroupInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getSubscriberGroup(string $id, ?array $options = null): ?SubscriberGroup
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/subscriber-groups/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return SubscriberGroup::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
