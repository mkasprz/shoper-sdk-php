...
namespace Shoper\Sdk\Rest\AuctionOrders;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\AuctionOrders\Requests\ListAuctionOrdersRequest;
use Shoper\Sdk\Rest\AuctionOrders\Types\ListAuctionOrdersResponse;
...
use Shoper\Sdk\Rest\AuctionOrders\Requests\AuctionOrderInsert;
use Shoper\Sdk\Rest\Types\AuctionOrder;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\AuctionOrders\Requests\AuctionOrderUpdate;

class AuctionOrdersClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListAuctionOrdersRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListAuctionOrdersResponse
    ...
    public function listAuctionOrders(ListAuctionOrdersRequest $request = new ListAuctionOrdersRequest(), ?array $options = null): ?ListAuctionOrdersResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListAuctionOrdersResponse::fromJson($json);
            }
    ...
    /**
     * @param AuctionOrderInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getAuctionOrder(string $id, ?array $options = null): ?AuctionOrder
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/auction-orders/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return AuctionOrder::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * @throws ShoperApiException
     */
    public function updateAuctionOrder(string $id, AuctionOrderUpdate $request = new AuctionOrderUpdate(), ?array $options = null): int|AuctionOrder|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
