...
namespace Shoper\Sdk\Rest\ProductFiles;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\ProductFiles\Requests\ListProductFilesRequest;
use Shoper\Sdk\Rest\ProductFiles\Types\ListProductFilesResponse;
...
use Shoper\Sdk\Rest\ProductFiles\Requests\ProductFileInsert;
use Shoper\Sdk\Rest\Types\ProductFile;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\ProductFiles\Requests\ProductFileUpdate;

class ProductFilesClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListProductFilesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function listProductFiles(ListProductFilesRequest $request = new ListProductFilesRequest(), ?array $options = null): ?ListProductFilesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListProductFilesResponse::fromJson($json);
            }
    ...
    /**
     * @param ProductFileInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getProductFile(string $id, ?array $options = null): ?ProductFile
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/product-files/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ProductFile::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
