...
namespace Shoper\Sdk\Rest\Progresses;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\Progresses\Requests\ListProgressesRequest;
use Shoper\Sdk\Rest\Progresses\Types\ListProgressesResponse;
...
use Shoper\Sdk\Rest\Progresses\Requests\ProgressInsert;
use Shoper\Sdk\Rest\Types\Progress;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\Progresses\Requests\ProgressUpdate;

class ProgressesClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListProgressesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function listProgresses(ListProgressesRequest $request = new ListProgressesRequest(), ?array $options = null): ?ListProgressesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListProgressesResponse::fromJson($json);
            }
    ...
    public function getProgress(string $id, ?array $options = null): ?Progress
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/progresses/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updateProgress(string $id, ProgressUpdate $request = new ProgressUpdate(), ?array $options = null): int|Progress|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deleteProgress(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
