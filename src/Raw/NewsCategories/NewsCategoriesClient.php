...
namespace Shoper\Sdk\Rest\NewsCategories;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\NewsCategories\Requests\ListNewsCategoriesRequest;
use Shoper\Sdk\Rest\NewsCategories\Types\ListNewsCategoriesResponse;
...
use Shoper\Sdk\Rest\NewsCategories\Requests\NewsCategoryInsert;
use Shoper\Sdk\Rest\Types\NewsCategory;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\NewsCategories\Requests\NewsCategoryUpdate;

class NewsCategoriesClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListNewsCategoriesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListNewsCategoriesResponse
    ...
    public function listNewsCategories(ListNewsCategoriesRequest $request = new ListNewsCategoriesRequest(), ?array $options = null): ?ListNewsCategoriesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListNewsCategoriesResponse::fromJson($json);
            }
    ...
    /**
     * @param NewsCategoryInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getNewsCategory(string $id, ?array $options = null): ?NewsCategory
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/news-categories/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updateNewsCategory(string $id, NewsCategoryUpdate $request = new NewsCategoryUpdate(), ?array $options = null): int|NewsCategory|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deleteNewsCategory(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
