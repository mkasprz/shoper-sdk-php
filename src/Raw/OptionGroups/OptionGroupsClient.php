...
namespace Shoper\Sdk\Rest\OptionGroups;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\OptionGroups\Requests\ListOptionGroupsRequest;
use Shoper\Sdk\Rest\OptionGroups\Types\ListOptionGroupsResponse;
...
use Shoper\Sdk\Rest\OptionGroups\Requests\OptionGroupInsert;
use Shoper\Sdk\Rest\Types\OptionGroup;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\OptionGroups\Requests\OptionGroupUpdate;

class OptionGroupsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListOptionGroupsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListOptionGroupsResponse
    ...
    public function listOptionGroups(ListOptionGroupsRequest $request = new ListOptionGroupsRequest(), ?array $options = null): ?ListOptionGroupsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListOptionGroupsResponse::fromJson($json);
            }
    ...
    /**
     * @param OptionGroupInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getOptionGroup(string $id, ?array $options = null): ?OptionGroup
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/option-groups/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updateOptionGroup(string $id, OptionGroupUpdate $request = new OptionGroupUpdate(), ?array $options = null): int|OptionGroup|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deleteOptionGroup(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
