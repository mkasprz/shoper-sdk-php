...
namespace Shoper\Sdk\Rest\PromotionCodes;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\PromotionCodes\Requests\ListPromotionCodesRequest;
use Shoper\Sdk\Rest\PromotionCodes\Types\ListPromotionCodesResponse;
...
use Shoper\Sdk\Rest\PromotionCodes\Requests\PromotionCodeInsert;
use Shoper\Sdk\Rest\Types\PromotionCode;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\PromotionCodes\Requests\PromotionCodeUpdate;
...
class PromotionCodesClient
{
    ...
    /**
     * @param ListPromotionCodesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListPromotionCodesResponse
    ...
    public function listPromotionCodes(ListPromotionCodesRequest $request = new ListPromotionCodesRequest(), ?array $options = null): ?ListPromotionCodesResponse
    {
        ...
        if ($request->filtersName != null) {
            $query['filters[name]'] = $request->filtersName;
        }
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/promotion-codes",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListPromotionCodesResponse::fromJson($json);
            }
    ...
    /**
     * @param PromotionCodeInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getPromotionCode(string $id, ?array $options = null): ?PromotionCode
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/promotion-codes/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updatePromotionCode(string $id, PromotionCodeUpdate $request = new PromotionCodeUpdate(), ?array $options = null): int|PromotionCode|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deletePromotionCode(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
