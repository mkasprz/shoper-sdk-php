...
namespace Shoper\Sdk\Rest\OrderTransactions;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\OrderTransactions\Requests\ListOrderTransactionsRequest;
use Shoper\Sdk\Rest\OrderTransactions\Types\ListOrderTransactionsResponse;
...
use Shoper\Sdk\Rest\OrderTransactions\Requests\OrderTransactionInsert;
use Shoper\Sdk\Rest\Types\OrderTransaction;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\OrderTransactions\Requests\OrderTransactionUpdate;

class OrderTransactionsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListOrderTransactionsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListOrderTransactionsResponse
    ...
    public function list(ListOrderTransactionsRequest $request = new ListOrderTransactionsRequest(), ?array $options = null): ?ListOrderTransactionsResponse
    {
        ...
        if ($request->filtersRefundId != null) {
            $query['filters[refund_id]'] = $request->filtersRefundId;
        }
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/order-transactions",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListOrderTransactionsResponse::fromJson($json);
            }
    ...
    /**
     * @param OrderTransactionInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function get(string $id, ?array $options = null): ?OrderTransaction
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/order-transactions/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
