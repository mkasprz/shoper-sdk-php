...
namespace Shoper\Sdk\Rest\Warehouses;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\Warehouses\Requests\ListWarehousesRequest;
use Shoper\Sdk\Rest\Warehouses\Types\ListWarehousesResponse;
...
use Shoper\Sdk\Rest\Warehouses\Requests\WarehouseInsert;
use Shoper\Sdk\Rest\Types\Warehouse;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\Warehouses\Requests\WarehouseUpdate;
...
class WarehousesClient
{
    ...
    public function listWarehouses(ListWarehousesRequest $request = new ListWarehousesRequest(), ?array $options = null): ?ListWarehousesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListWarehousesResponse::fromJson($json);
            }
    ...
    public function getWarehouse(string $id, ?array $options = null): ?Warehouse
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/warehouses/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updateWarehouse(string $id, WarehouseUpdate $request = new WarehouseUpdate(), ?array $options = null): int|Warehouse|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deleteWarehouse(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
