...
namespace Shoper\Sdk\Rest\Attributes;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\Attributes\Requests\ListAttributesRequest;
use Shoper\Sdk\Rest\Attributes\Types\ListAttributesResponse;
...
class AttributesClient
{
    ...
    /**
     * @param ListAttributesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function listAttributes(ListAttributesRequest $request = new ListAttributesRequest(), ?array $options = null): ?ListAttributesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListAttributesResponse::fromJson($json);
            }
    ...
    public function getAttribute(string $id, ?array $options = null): ?Attribute
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/attributes/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
     ...
     * @throws ShoperApiException
     */
    public function updateAttribute(string $id, AttributeUpdate $request = new AttributeUpdate(), ?array $options = null): int|Attribute|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
     ...
     * @throws ShoperApiException
     */
    public function deleteAttribute(string $id, ?array $options = null): ?int
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
