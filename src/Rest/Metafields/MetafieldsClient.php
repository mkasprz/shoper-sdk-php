...
namespace Shoper\Sdk\Rest\Metafields;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\Metafields\Requests\ListMetafieldsRequest;
use Shoper\Sdk\Rest\Metafields\Types\ListMetafieldsResponse;
...
use Shoper\Sdk\Rest\Metafields\Requests\MetafieldInsert;
use Shoper\Sdk\Rest\Types\Metafield;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\Metafields\Requests\MetafieldUpdate;

class MetafieldsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    public function listMetafields(string $object, ListMetafieldsRequest $request = new ListMetafieldsRequest(), ?array $options = null): ?ListMetafieldsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListMetafieldsResponse::fromJson($json);
            }
    ...
    public function getMetafield(string $object, string $id, ?array $options = null): ?Metafield
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/metafields/{$object}/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
