...
namespace Shoper\Sdk\Rest\AttributeGroups;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\AttributeGroups\Requests\ListAttributeGroupsRequest;
use Shoper\Sdk\Rest\AttributeGroups\Types\ListAttributeGroupsResponse;
...
use Shoper\Sdk\Rest\AttributeGroups\Requests\AttributeGroupInsert;
use Shoper\Sdk\Rest\Types\AttributeGroup;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\AttributeGroups\Requests\AttributeGroupUpdate;

class AttributeGroupsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListAttributeGroupsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function listAttributeGroups(ListAttributeGroupsRequest $request = new ListAttributeGroupsRequest(), ?array $options = null): ?ListAttributeGroupsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListAttributeGroupsResponse::fromJson($json);
            }
    ...
    /**
     * @param AttributeGroupInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function createAttributeGroup(AttributeGroupInsert $request, ?array $options = null): int|AttributeGroup|null
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('integer', AttributeGroup::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
    ...
    public function getAttributeGroup(string $id, ?array $options = null): ?AttributeGroup
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/attribute-groups/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
    ...
    /**
     * @param string $id
     * @param AttributeGroupUpdate $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws ShoperApiException
     */
    public function updateAttributeGroup(string $id, AttributeGroupUpdate $request = new AttributeGroupUpdate(), ?array $options = null): bool|AttributeGroup|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('bool', AttributeGroup::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * @throws ShoperApiException
     */
    public function deleteAttributeGroup(string $id, ?array $options = null): ?bool
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
