...
namespace Shoper\Sdk\Rest\WarehouseRelocations;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\WarehouseRelocations\Requests\ListWarehouseRelocationsRequest;
use Shoper\Sdk\Rest\WarehouseRelocations\Types\ListWarehouseRelocationsResponse;
...
use Shoper\Sdk\Rest\WarehouseRelocations\Requests\WarehouseRelocationInsert;
...
class WarehouseRelocationsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListWarehouseRelocationsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListWarehouseRelocationsResponse
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    ...
                    path: "webapi/rest/warehouse-relocations",
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListWarehouseRelocationsResponse::fromJson($json);
            }
    ...
    /**
     * @param WarehouseRelocationInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function create(WarehouseRelocationInsert $request = new WarehouseRelocationInsert(), ?array $options = null): int|WarehouseRelocation|null
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('integer', WarehouseRelocation::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
    ...
    public function get(string $id, ?array $options = null): ?WarehouseRelocation
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/warehouse-relocations/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return WarehouseRelocation::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
