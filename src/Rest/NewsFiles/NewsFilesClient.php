...
namespace Shoper\Sdk\Rest\NewsFiles;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\NewsFiles\Requests\ListNewsFilesRequest;
use Shoper\Sdk\Rest\NewsFiles\Types\ListNewsFilesResponse;
...
use Shoper\Sdk\Rest\NewsFiles\Requests\NewsFileInsert;
use Shoper\Sdk\Rest\Types\NewsFile;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\NewsFiles\Requests\NewsFileUpdate;

class NewsFilesClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListNewsFilesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function list(ListNewsFilesRequest $request = new ListNewsFilesRequest(), ?array $options = null): ?ListNewsFilesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListNewsFilesResponse::fromJson($json);
            }
    ...
    public function get(string $id, ?array $options = null): ?NewsFile
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/news-files/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
