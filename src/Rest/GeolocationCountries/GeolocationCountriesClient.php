...
namespace Shoper\Sdk\Rest\GeolocationCountries;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\GeolocationCountries\Requests\ListGeolocationCountriesRequest;
use Shoper\Sdk\Rest\GeolocationCountries\Types\ListGeolocationCountriesResponse;
...
use Shoper\Sdk\Rest\Types\GeolocationCountry;

class GeolocationCountriesClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListGeolocationCountriesRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListGeolocationCountriesResponse
    ...
    public function list(ListGeolocationCountriesRequest $request = new ListGeolocationCountriesRequest(), ?array $options = null): ?ListGeolocationCountriesResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListGeolocationCountriesResponse::fromJson($json);
            }
    ...
    public function get(string $id, ?array $options = null): ?GeolocationCountry
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/geolocation-countries/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return GeolocationCountry::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
