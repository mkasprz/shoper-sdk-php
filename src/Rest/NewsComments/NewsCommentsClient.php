...
namespace Shoper\Sdk\Rest\NewsComments;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\NewsComments\Requests\ListNewsCommentsRequest;
use Shoper\Sdk\Rest\NewsComments\Types\ListNewsCommentsResponse;
...
use Shoper\Sdk\Rest\NewsComments\Requests\NewsCommentInsert;
use Shoper\Sdk\Rest\Types\NewsComment;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\NewsComments\Requests\NewsCommentUpdate;

class NewsCommentsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListNewsCommentsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListNewsCommentsResponse
    ...
    public function listNewsComments(ListNewsCommentsRequest $request = new ListNewsCommentsRequest(), ?array $options = null): ?ListNewsCommentsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListNewsCommentsResponse::fromJson($json);
            }
    ...
    /**
     * @param NewsCommentInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function getNewsComment(string $id, ?array $options = null): ?NewsComment
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/news-comments/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return NewsComment::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
