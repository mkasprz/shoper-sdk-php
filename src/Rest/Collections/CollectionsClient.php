...
namespace Shoper\Sdk\Rest\Collections;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\Collections\Requests\ListCollectionsRequest;
use Shoper\Sdk\Rest\Collections\Types\ListCollectionsResponse;
...
use Shoper\Sdk\Rest\Collections\Requests\CollectionInsert;
use Shoper\Sdk\Rest\Types\Collection;
use Shoper\Sdk\Rest\Core\Json\JsonDecoder;
use Shoper\Sdk\Rest\Core\Types\Union;
use Shoper\Sdk\Rest\Collections\Requests\CollectionUpdate;
...
class CollectionsClient
{
    ...
    /**
     * @param CollectionInsert $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
    ...
    public function createCollection(CollectionInsert $request = new CollectionInsert(), ?array $options = null): int|Collection|null
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('integer', Collection::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
    ...
    public function getCollection(string $id, ?array $options = null): ?Collection
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/collections/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
    ...
    /**
     * @param string $id
     * @param CollectionUpdate $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws ShoperApiException
     */
    public function updateCollection(string $id, CollectionUpdate $request = new CollectionUpdate(), ?array $options = null): bool|Collection|null
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return JsonDecoder::decodeUnion($json, new Union('bool', Collection::class)); // @phpstan-ignore-line
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * @throws ShoperApiException
     */
    public function deleteCollection(string $id, ?array $options = null): ?bool
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
