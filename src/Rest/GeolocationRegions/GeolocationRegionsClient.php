...
namespace Shoper\Sdk\Rest\GeolocationRegions;

use Psr\Http\Client\ClientInterface;
use Shoper\Sdk\Rest\Core\Client\RawClient;
use Shoper\Sdk\Rest\GeolocationRegions\Requests\ListGeolocationRegionsRequest;
use Shoper\Sdk\Rest\GeolocationRegions\Types\ListGeolocationRegionsResponse;
...
use Shoper\Sdk\Rest\Types\GeolocationRegion;

class GeolocationRegionsClient
{
    /**
     * @var array{
     *   baseUrl?: string,
    ...
    /**
     * @param ListGeolocationRegionsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * } $options
     * @return ?ListGeolocationRegionsResponse
    ...
    public function list(ListGeolocationRegionsRequest $request = new ListGeolocationRegionsRequest(), ?array $options = null): ?ListGeolocationRegionsResponse
    {
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return ListGeolocationRegionsResponse::fromJson($json);
            }
    ...
    public function get(string $id, ?array $options = null): ?GeolocationRegion
    {
        ...
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
                    path: "webapi/rest/geolocation-regions/{$id}",
                    method: HttpMethod::GET,
                ),
                $options,
            );
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return GeolocationRegion::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new ShoperException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
